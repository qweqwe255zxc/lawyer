...
/**
 * Таблица: строка на всю ширину, реестром. Так помещается больше и
 * читается как список дел, а не как галерея. Вариант по умолчанию в
 * «Экономе». (Раньше назывался "register" — имя было буквальным
 * переводом с "реестра", но не говорило ничего о самой раскладке;
 * "table" — то же имя, что уже носит табличный variant у Features и
 * Pricing.)
 *
 ...
 * одного места.
 */
export function Table(props: GallerySection) {
  const { items } = props;
      ...
      <ul>
        {items.map((item, index) => (
          ...
          >
            ...
            >
              <span className="mr-2 text-caption uppercase md:hidden">Год:</span>
              {item.year}
            </p>
          </li>
        ))}
