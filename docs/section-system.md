...
`title?`, `lead?`, `nav?`, `iconShape?`, `headerAlign?`. Не все
...
| `split` | текст слева, фото/виджет справа (1/6/5); потолок H1 72px | `image` или `widget`, иначе откат на `type-only` |
...
| `bento` | эйброу по центру, 2 колонки: первая — акцентная рамка, последняя — с мягкой тонировкой фона (`bg-fg/6%`, не полная инверсия поверхности), остальные `framed`. Асимметрию spans не повторяет (см. раздел 6, «Бизнес — SaaS») | `icon`; `text` заменяет `label` |
...
| `banner?: TeamBanner` | нет | — | баннер «Хотите к нам?» под сеткой; читают `photo-cards`/`badge-avatars`/`tags-cards`, каждый своим `tone` |
...
| `wide` | Та же шапка+аккордеон, `Container width="page"` (1240px). Имя буквальное — своей двухколоночной раскладки у варианта нет и не было, только контейнер шире |
...
| `centered` | эйброу пилюлей, всё по центру в одну колонку |
| `left` | по левому краю, эйброу точкой, без второй колонки |
| `boxed` | контент в `Card variant="elevated"` поверх акцентной заливки |
...
| `monogram` | плашка с инициалом на акцентном градиенте |
| `centered` | знак и навигация в две строки по центру, без кнопки и бургера |
| `glass` | стеклянный фон включён с первого кадра (не по скроллу) |
| `split` | навигация в два кластера, вордмарк — пункт между ними |
...
**Footer** (`siteConfig.footer.variant`, дефолт `"default"`) — поля
`columns?`, `social?`, `newsletter?` читают только варианты, которым
нужны:

| Значение | Макет |
|---|---|
| `default` | знак+тег слева, `nav` по центру, реквизиты справа |
| `bold` | вордмарк+описание+`social` слева, `columns` справа |
| `classic` | тот же состав, мягче типографика |
| `compact` | одна строка: знак+копирайт слева, `links` справа |
| `monogram` | плашка на градиенте (или сплошном акценте), `columns`+`newsletter` (форма `method="get"`); `footer.monogramBackground: "gradient"\|"surface"` — раньше это были два отдельных файла (`gradient`/`glass`), отличавшихся только фоном плашки и поверхностью секции |
| `centered` | знак, `nav`, копирайт, `social` — всё по центру |
| `split` | `nav` в два кластера вокруг вордмарка, копирайт+`social` снизу |

Общие куски: `parts/SocialLinks.tsx`, `parts/FooterColumns.tsx`,
`parts/BottomBar.tsx`, `parts/NewsletterForm.tsx`.
...
| `framed` | `ui-card border` | **карточка по умолчанию**: Эконом — hairline без тени, Стандарт — своя поверхность+тень+радиус 16px, Premium — приглушённая акцентная рамка |
...
| `ui-media` | скругление медиа внутри карточки (без тени) | фото Features/Pricing, аватар Team |
...
Полная таблица значений по тарифам (Эконом/Стандарт/Premium) —
...
Тарифы (`econom`/`standard`/`premium`) подробно описаны в
...
Переключает пластику (тени, радиусы, поверхности карточек, подсветку
секций, плашки под иконками, стеклянный хедер, поля-коробки) и
...
### Бизнес — SaaS (референс Aeterna)

Самый дальний отход от базы; часть требований не покрыта компонентами.
`preset: "standard"` (или `"premium"` без теней).
